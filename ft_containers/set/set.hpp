...
public:
	...
	typedef map_iterator_reverse<value_type >		reverse_iterator;
	typedef map_iterator_reverse<const value_type >		const_reverse_iterator;
	#define BLACK 0
	#define RED 1
private:
	Node			*root;
...
explicit	set(const key_compare& _compp = key_compare(),
...
set(const set &copy) : root(NULL), _size(0), alloc(std::allocator<Node >()), _comp(std::less<T >())
{
	this->_end_left = this->alloc.allocate(1);
	this->_end_right = this->alloc.allocate(1);
	this->alloc.construct(this->_end_left);
...
pair<iterator, bool>		insert(const value_type &val)
{
	...
	this->alloc.construct(n, Node(val));
	n->_left = NULL;
	n->_right = NULL;
	n->_root = y;
	++this->_size;
